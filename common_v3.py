...
# def check_verbal_adjective(word_data):
# #     print('Running check_adjective')
#     '''
#     Functionality:
#         1. Check if dependency data is any of the following - 'card', 'mod', 'meas', 'ord', 'intf'
...
#     if word_data_list[4] != '':
#         rel = word_data_list[4].strip().split(':')[1]
#         if rel in constant.VERBAL_ADJECTIVE:
#             return True
...
#     print('Running check_nominal_verb')
    ...
    if word_data[4].strip() != '':
        ...
        for tag in tags:
            # print(tags,'tags')
            if tag == ('cat', 'v') and relation in constant.NOMINAL_VERB_DEPENDENCY:
...
def preprocess_postposition_new(concept_type, np_data, words_info, verb_data):
    ...
    if data_case in ('k1', 'pk1'):
        ...
        if data_seman and data_seman!=''and data_seman.split()[0] in ("anim", "per"):
            if clean(root_main) in constant.reciprocal_verbs:
                ppost = 'se'
            else:
                ppost = 'ko'
        else:
...
# def process_verbal_adjective(verbal_adjectives,processed_nouns, words_info, verbs_data):
#     for verb in verbs_data:
#        if len(verb[4]) > 0 and verb[4].strip().split(':')[1] == 'main' or verb[4].strip().split(':')[1] == 'rcelab' or verb[4].strip().split(':')[1] == 'rcdelim':
#             main_verb = verb
#             break

#     for nominal_verb in verbal_adjectives:
...
def process_pronouns(pronouns, processed_nouns, processed_indeclinables, words_info, verbs_data):
    ...
    for pronoun in pronouns:
        ...
        if is_kim(term):
            ...
            processed_pronouns, processed_indeclinables = process_kim(index, relation, anim, gnp,case, pronoun, words_info,
                                                                      main_verb, processed_pronouns, processed_indeclinables, processed_nouns)
        elif is_yax(term):
            gender, number, person, case = get_default_GNP()
            processed_pronouns, processed_indeclinables = process_yax(index, relation, anim, gnp,case, pronoun, words_info,
                                                                      main_verb, processed_pronouns, processed_indeclinables, processed_nouns)
...
def process_verbs(verbs_data, seman_data, depend_data, sentence_type, spkview_data, processed_nouns, processed_pronouns,index_data, words_info, reprocess=False):
    '''
    Functionality:
    ...
    for concept in verbs_data:
        ...
        if verb_type == 'nonfinite':
            ...
            verb, aux_verbs = process_verb(concept, seman_data, depend_data, sentence_type, spkview_data, processed_nouns, processed_pronouns,index_data, reprocess)
            processed_verbs.append(to_tuple(verb))
...
# def process_verbal_adjective(adjectives_data,processed_nouns, words_info,processed_adjectives, processed_verbs,verbs_data):
#     # verbal_adjectives = []
#     # for verb in verbs_data:
#     #     if len(verb[4]) > 0 and verb[4].strip().split(':')[1] == 'main' or verb[4].strip().split(':')[1] == 'rcelab' or verb[4].strip().split(':')[1] == 'rcdelim':
#     #         main_verb = verb
...
#     for adjectives in adjectives_data:
#         index = adjectives[0]
#         term = clean(adjectives[1])
#         gender = 'm'
#         number = 's'
#         person = 'a'
#         category = 'n'
...
#         case = 'o'
#         # postposition = ''
#         log_msg = f'{term} identified as verbal adjective, re-identified as other word and processed as common adjective with index {index} gen:{gender} num:{number} person:{person} case:{case}'

#         relation = ''
#         if adjectives[4] != '':
#             relation = adjectives[4].strip().split(':')[1]
#         tags = find_tags_from_dix_as_list(term)
#         for tag in tags:
#             if (tag[0] == 'cat' and tag[1] == 'v'):
#                 # noun_type = ''
...
#                 if relation in ('rvks'):
#                     term = term + 'wA_huA'
#                 log_msg = f'{term} processed as verbal adjective with index {index} gen:{gender} num:{number} person:{person}  case:{case}'
#                 break
#         adjective = (index, term, category, case, gender, number)
#         # processed_noun.append(noun)
#         processed_adjectives.append(adjective)
#         log(log_msg)
#     return processed_adjectives

def process_kim(index, relation, anim, gnp,case, pronoun, words_info, main_verb, processed_pronouns, processed_indeclinables, processed_nouns):
...
def process_yax(index, relation, anim, gnp,case, pronoun, words_info, main_verb, processed_pronouns, processed_indeclinables, processed_nouns):
...
def construction_row(i,construction_data1,depend_data):
    dep1=depend_data[i]
...
def construction_row_cp(i,construction_data1,depend_data):
    dep1=depend_data[i]
    for j, text in enumerate(construction_data1):
        if 'kriyAmUla' in text or 'verbalizer' in text:
            # op_indexes.append(i)
            depend_data[j]=dep1
        # elif 'start' in text:
        #     depend_data[j]=dep1
        # elif 'end' in text:
        #     depend_data[j]=dep1
    print(depend_data,'dpn')
    return depend_data

def new_to_old_convert_construction_cp(construction_data,conj_concept):
    op_index=[]
    construction_data1=''
    print(construction_data,conj_concept)
...
def new_to_old_convert_construction_span(index_data,construction_data,span_concept):
    ...
    op_sub_ind=[]
    construction_data1=''
    print(construction_data,span_concept)
...
def process_construction(processed_words, construction_data1, depend_data, gnp_data, index_data,conj_concept):
    ...
    construction_data=new_to_old_convert_construction_conj_dis(index_data,construction_data1,conj_concept)
    ...
    if construction_data != '*nil' and len(construction_data) > 0:
        ...
        for cons in construction:
            ...
            if 'conj' in conj_type or 'disjunct' in conj_type:
                cnt_m = 0
                cnt_f = 0
                PROCESS = False
                for i in index:
                ...
                for i in index:
                    if i == update_index:
                        ...
                        if 'conj' in conj_type:
                            temp = (a, 'Ora')
                            print(temp,'varsk')
                        elif 'disjunct' in conj_type:
                            temp = (a, 'yA')
                        break
                    else:
                        temp = (a, ',')
...
def process_construction_span(processed_words, construction_data1, depend_data, gnp_data, index_data,span_concept):
    construction_dict.clear()
    process_data = processed_words
    dep_gender_dict = {}
    a = 'after'
...
def process_verb(concept: Concept, seman_data, dependency_data, sentence_type, spkview_data, processed_nouns, processed_pronouns,index_data, reprocessing):
    """
    ...
    *Aux root and Aux TAM identified from auxillary mapping File
    ...
    verb = process_main_verb(concept, seman_data, dependency_data, sentence_type, processed_nouns, processed_pronouns,index_data, reprocessing)
...
def process_nonfinite_verb(concept, seman_data, depend_data, sentence_type, processed_nouns, processed_pronouns,index_data, words_info):
    ...
    gender, number, person = getVerbGNP_new(verb.term, full_tam, is_cp, seman_data, depend_data, sentence_type, processed_nouns, processed_pronouns,index_data)
    verb.gender = gender
    verb.number = number
    verb.person = person
...
def process_main_verb(concept: Concept, seman_data, dependency_data, sentence_type, processed_nouns, processed_pronouns,index_data, reprocessing):
    """
    ...
    verb.gender, verb.number, verb.person = getVerbGNP_new(concept.term, full_tam, is_cp, seman_data, dependency_data, sentence_type, processed_nouns, processed_pronouns,index_data)
    return verb
    # else:
    #     return None
...
def getVerbGNP_new(concept_term, full_tam, is_cp, seman_data, depend_data, sentence_type, processed_nouns, processed_pronouns,index_data):
...
#     print('Running has_ques_mark')
    ...
    # print(POST_PROCESS_OUTPUT,'ppo1')
    print(sentence_type,'st')
...
def rearrange_sentence(fulldata, coref_list):
    ...
    for item in fulldata:
        ...
        if index in coref_dict:
            ...
            print(index,'coref_dict')
            word += f" ({coref_dict[index]})"
...
#     elif data_case == 'k2': #if CP present, and if concept is k2 for verb of CP, and the verb is not in specific list, then kA
#         if is_CP(np_data[1]):
#             first_word=np_data[1].split('+')[0]
#             if first_word in constant.kriyAmUla:
#                 ppost='kA'
#         elif data_seman and data_seman!=''and data_seman.split()[0] in ("anim", "per"):
#             if clean(root_main) in constant.reciprocal_verbs:
#                 ppost = 'se'
#             else:
#                 ppost = 'ko'
...
#         case, postposition = preprocess_postposition_new('noun', nominal_verb, words_info, main_verb)
#         # concept_dep_head = nominal_verb[4].strip().split(':')[0]
#         # concept_dep_val = nominal_verb[4].strip().split(':')[1]
#         # nominal_verb = Concept(index=concept[0], term=concept[1], dependency=concept[4])
#         # if(concept_dep_val == 'vk2'):
#         #     update_additional_words_dict(int(concept_dep_head), 'after', 'ki')
#         is_cp = is_CP(term)
#         print(term)
#         if is_cp:
#             # if not reprocess:
#             CP = process_main_CP(index, term)
#             if CP != [] and CP[2] == 'n':
#                 log(f'{CP[1]} processed as noun with index {CP[0]} case:d gen:{CP[4]} num:{CP[5]} per:{CP[6]}, noun_type:{CP[7]}, default postposition:{CP[8]}.')
#                 processed_noun.append(tuple(CP))

#         if '+' in nominal_verb[1]:
#             term=nominal_verb[1].split('+')[1]
#             term = clean(term)
#             tags = find_tags_from_dix_as_list(term)
#             for tag in tags:
#                 if (tag[0] == 'cat' and tag[1] == 'v'):
#                     noun_type = 'vn'
#                     category='vn'
#                     if relation in ('k2', 'rt', 'rh','rblpk','rblak','rblsk','k1s'):
#                         term = term + 'nA'
#                         log_msg = f'{term} processed as nominal verb with index {index} gen:{gender} num:{number} person:{person} noun_type:{noun_type} case:{case} and postposition:{postposition}'
#                         break
#                     elif relation in ('k1'):
...
#                 elif spkview_data == "distal" and number=='p':
#                     word='vaha'
#                 elif spkview_data == "proximal" and number=='s':
#                     word = 'yaha'
#                 elif spkview_data == "proximal" and number=='p':
#                     word = 'yaha'
#                 else:
#                     word = term
...
#     if sentence_type[1:] in ("yn_interrogative", "yn_interrogative_negative", "pass-yn_interrogative", ):
#         return 'kyA ' + POST_PROCESS_OUTPUT + ' ?'
#     elif sentence_type[1:] in ("interrogative","Interrogative","pass-interrogative"):
#         return POST_PROCESS_OUTPUT + ' ?'
#     elif sentence_type[1:] in ('pass_affirmative','affirmative', 'Affirmative', 'negative', 'Negative', 'imperative', 'Imperative',"fragment","term","title","heading"):
#         return POST_PROCESS_OUTPUT + ' |'
...
# def handle_unprocessed(index_data, outputData, processed_nouns):
#     """Swapping gender info that does not exist in the dictionary."""
#     output_data = outputData.strip().split(" ")
#     has_changes = False
#     print(index_data,output_data,processed_nouns)
#     # Iterate over the output data and find corresponding processed nouns
#     for idx, data in enumerate(output_data):
#         if data[0] == '#':
#             for i in range(len(processed_nouns)):
#                 ind = round(processed_nouns[i][0])
#                 if ind in index_data:  # Map index_data with output_data
#                     if processed_nouns[i][7] not in ('proper', 'NC', 'CP_noun', 'abs', 'vn'):
#                         has_changes = True
#                         temp = list(processed_nouns[i])
#                         temp[4] = 'f' if processed_nouns[i][4] == 'm' else 'm'
...
# def extract_gnp_noun(noun_data1):
# #     print('Running extract_gnp_noun')
#     print(noun_data1,'ndd')
#     gender = 'm'
#     number = 's'
#     person = 'a'

#     if len(noun_data1):
#         noun_term = noun_data1[1]
#         if check_is_digit(noun_term):
#             noun_term = noun_term
#         elif '+' in noun_term:
#             cn_terms = noun_term.strip().split('+')
...
#         # Setting gender
#         semantic_data = data[2].strip()
#         # seman_info_lst = seman_data.split()
#         if len(semantic_data) > 0:
#             if 'male' in semantic_data:
#                 gender = 'm'
#             elif 'female' in semantic_data:
#                 gender = 'f'
#             # elif 'pl' in semantic_data:
...
#     final_words = [x[1].strip() for x in fulldata]
#     r_s=" ".join(final_words)
# #     print('rearrange_sentence : ',r_s)
#     return r_s
...
# def process_coref(processed_words,input_text):
#     coref_list = []
#     folder_path = sys.argv[1].split('/')[0]
#     for i in range(len(input_text)):
#         sub_coref_list=[]
#         if 'coref' in input_text[i] and '.' in input_text[i]:
#             sub_coref_list.append(i)
#             file_name_line = input_text[i]
#             file_name = file_name_line.split('.')[0]
#             digit = file_name_line.split('.')[1].split(':')[0]
...
#     return coref_list if coref_list else None
# def reset_global_dicts():
#     global additional_words_dict, processed_postpositions_dict, construction_dict, spkview_dict, MORPHO_SEMANTIC_DICT
#     additional_words_dict.clear()
#     processed_postpositions_dict.clear()
